 ...
 * 实名认证－证件照片
 */
public class SafetyPicture extends SwipeBackActivity implements View.OnClickListener {
    ...
    @BindView(R.id.ll_identity_scsfz)
    LinearLayout ll_identity_scsfz;
    @BindView(R.id.ll_identity_sfzzm)
    LinearLayout ll_identity_sfzzm;
    @BindView(R.id.ll_identity_sfzfm)
    LinearLayout ll_identity_sfzfm;
   ...
   // private int djgtp = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.user_safety_identity_auth_picture);
        context = this;
    ...
    private void initView() {
        ...
        tv_head_title.setText(R.string.safety_sfzxx);
        btn_head_back.setVisibility(View.VISIBLE);
        btn_head_back.setOnClickListener(this);

       /* ll_identity_scsfz.setOnClickListener(this);
        ll_identity_sfzzm.setOnClickListener(this);
        ll_identity_sfzfm.setOnClickListener(this);*/

        img_identity_1.setOnClickListener(this);
        img_identity_2.setOnClickListener(this);
        img_identity_3.setOnClickListener(this);
        ...
        mUpdateImageSubscriberOnNextListener = new SubscriberOnNextListener<FileactionResult>() {
            ...
            public void onNext(FileactionResult o) {
                ...
                {
                    case 0:
                        loadImg = o.getFileName();
                        break;
                    case 1:
                        frontalImg = o.getFileName();
                        break;
                    case 2:
                        backImg = o.getFileName();
                        break;
                }
    ...
    private void initImageView(IdentityAuthResult identityAuthResult)
    {
      ...
      if(!frontalImg.equals(""))
        ...
        if(!backImg.equals(""))
        {
            ...
            ImageLoadTask mImageBackLoadTask = new ImageLoadTask(handler);
            mImageBackLoadTask.execute(identityAuthResult.getFileBasePath()+identityAuthResult.getBackImg()+"&rs=1",UserDao.getInstance().getToken(),userid);
            mImageBackLoadTask.setImageView(img_identity_2);

        }

        if(!loadImg.equals(""))
        {
            ImageLoadTask mImageLoadTask = new ImageLoadTask(handler);
            mImageLoadTask.execute(identityAuthResult.getFileBasePath()+identityAuthResult.getLoadImg()+"&rs=1",UserDao.getInstance().getToken(),userid);
            mImageLoadTask.setImageView(img_identity_3);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.img_identity_1:
                ...
                userSelect.show();
                break;
            case R.id.img_identity_2:
                mClickType = 1;
                imageView = img_identity_2;
                userSelect.show();
                break;
            case R.id.img_identity_3:
                mClickType = 2;
                imageView = img_identity_3;
                userSelect.show();
                break;
            case R.id.bnt_commit:
                if (loadImg.equals("")) {
                    UIHelper.ToastMessage(context, getString(R.string.safety_scsczjz_xz_toast));
                    return;
                }
                if (frontalImg.equals("")) {
                    UIHelper.ToastMessage(context, getString(R.string.safety_sczjzmz_xz_toast));
                    return;
                }
                if (backImg.equals("")) {
                    UIHelper.ToastMessage(context, getString(R.string.safety_sczjfmz_xz_toast));
                    return;
                }
                depthIdentityAuth();
                break;
